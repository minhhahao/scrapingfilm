...
def xpath(selector, xpathValue):
    if isinstance(xpathValue, dict):
        ...
        if operator == 'and':
            ...
            for i in xpathValue['value']:
                if len(selector.xpath(i['element'][0])) > 0 and len(selector.xpath(i['element'][1])) > 0:
                    xpath_between = "%s[count(.|%s) = count(%s)]" % (i['element'][0], i['element'][1], i['element'][1])
                    return selector.xpath(xpath_between)
